...
- **A fixed array fits a declared slice.** Inside compile-time context a `[T : N]` value coerces to `&[T]`: every comptime result is materialized into static data anyway (§7.2), so the slice simply views it. A macro building an array literal therefore declares `-> &[T]` without hard-coding the length.
...
- **Macro bodies are type-checked like function bodies.** With typed parameters and a declared result, a body is checked statically under every §4 and §5 rule — overload resolution, reference explicitness, definite return against the declared type — before it ever runs, so a call to a missing function or a mismatched argument is an error at the macro, not a fault at its first use. The one difference from a function body is that it is compile-time context throughout: `#Type` values may be held in locals and passed around, and inline layouts (`#struct { ... }`) and nested `#` calls are plain expressions there. The body then runs in the compile-time interpreter, where a runtime fault (overflow, an exhausted budget, a failed `#read_file`) is a compile-time error at the call site; a body that produces something other than its declared type is an error at the call site too.
